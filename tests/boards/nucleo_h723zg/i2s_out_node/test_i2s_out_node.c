 ...
 */
BUILD_ASSERT(AUDIO_I2S_OUT_BLOCK_ALIGN >= CONFIG_DCACHE_LINE_SIZE,
	     "transfer blocks must be aligned to at least one d-cache line");
BUILD_ASSERT((AUDIO_I2S_OUT_BLOCK_ALIGN & (AUDIO_I2S_OUT_BLOCK_ALIGN - 1)) == 0,
 ...
 */
BUILD_ASSERT((AUDIO_I2S_OUT_BLOCK_BYTES(FRAME_SAMPLES) % AUDIO_I2S_OUT_BLOCK_ALIGN) == 0,
 ...
 */
BUILD_ASSERT(AUDIO_I2S_OUT_BLOCK_BYTES(FRAME_SAMPLES) >=
		     (size_t)FRAME_SAMPLES * AUDIO_I2S_WIRE_MAX_WORD_BYTES,
	     "one block must hold one frame");
BUILD_ASSERT(AUDIO_I2S_OUT_BLOCK_BYTES(2 * FRAME_SAMPLES) >
		     AUDIO_I2S_OUT_BLOCK_BYTES(FRAME_SAMPLES),
 ...
 */
BUILD_ASSERT(AUDIO_I2S_OUT_BLOCK_BYTES(FRAME_SAMPLES) !=
		     AUDIO_I2S_OUT_BLOCK_BYTES(CONFIG_AUDIO_PIPELINE_FRAME_SAMPLES),
...
static void *i2s_out_setup(void)
{
	printk("nucleo_h723zg i2s_out: %s, %zu byte blocks x %u\n", i2s_tx_dev->name,
	       (size_t)AUDIO_I2S_OUT_BLOCK_BYTES(FRAME_SAMPLES), TX_BLOCKS);
...
ZTEST(board_i2s_out_node, test_i2s_out_owns_transfer_blocks_dma_can_reach)
{
	...
	zassert_equal(i2s_out_slab.info.block_size, AUDIO_I2S_OUT_BLOCK_BYTES(FRAME_SAMPLES),
		      "the slab must carry the block size the node reports to the driver");
	zassert_equal(i2s_out_slab.info.num_blocks, TX_BLOCKS);
	zassert_equal(base % AUDIO_I2S_OUT_BLOCK_ALIGN, 0U,
...
ZTEST(board_i2s_out_node, test_i2s_out_configures_the_device_as_a_clock_target)
{
	...
	zassert_equal(cfg->block_size, AUDIO_I2S_OUT_BLOCK_BYTES(FRAME_SAMPLES));
